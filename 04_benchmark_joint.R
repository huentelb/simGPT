...
boot_ci_diff <- function(df, var) {
  ...
  summary <- lapply(names(stats), function(s) {
    ...
    qs <- replicate(reps, {
      ...
      x2 <- resampled[[var]][resampled$source == "Microsimulation"]
...
boot_seqind_diff <- function(seq_e, seq_m, var) {
  ...
  boot_diffs <- replicate(reps, {
    ...
    resampled1 <- seq_e[sample(nrow(seq_e), replace = TRUE),]
    resampled2 <- seq_m[sample(nrow(seq_m), replace = TRUE),]
...
tab1 <- tab1_agg %>% 
  ...
  select(Register, Microsimulation, diff, lb, ub)
...
# Define sequence object using the above characteristics
ac <- wcAggregateCases(gp[,2:paste0(max_age+2)])
ac

seq <- seqdef(gp[ac$aggIndex, 2:paste0(max_age+2)], # for max_age 100 to column 106, for max_age 66 to column 72
...
png(file = paste0(graph.folder, "seqrf.png"),
    width=w, height=h)
plot(srfchi, which.plot = "both", main = paste0("Random sample from joint data sources (n = 5,000)")) 
dev.off()
# summary(srfchi)
...
#### RFplot by clusters (n=5000 random sample per cluster) ####
w <- 7
h <- 7
...
c6 <- gp %>% 
  filter(chi == l6) #no random sampling because cluster <5000
...
gp_reg <- gp %>%
  mutate(
    source = factor(source, 
                    labels = c("Microsimulation",
                               "Register")),
    chi = factor(chi,
                 labels = c("C1 - Late 3-gen family",
                            "C2 - 4-gen family",
                            "C3 - 2-gen family",
                            "C4 - Non-parents late",
                            "C5 - Early 3-gen family",
                            "C6 - Non-parents early")),
  ) %>% 
  dplyr::select(chi, source)
...
ggaverage(fit_basic, terms = "source", ci_level = 0.95) %>%
  plot() 
