...
# 1953 & 2000 BIRTH COHORT - AGE RANGE 0-100
...
load(paste0(folder.baseseed, "gp1953100.RData"))
gp53 <- gp
...
seq53 <- seqdef(gp53, 6:paste0(max_age+6), # for max_age 100 to column 106, for max_age 66 to column 72
              labels = gplabels,  
              cnames = ages, 
              tick.last = TRUE, 
              xtstep = 5, 
...
indic53 <- seqindic(seq53, indic=c("lgth", "visited", "visitp", "transp", # length, states visited + prop, no. transitions
...
comp_occ <- round(seqCompare(seq53, seqdata2 = seq00, stat = "all", 
                             method = "OMspell", sm = "INDELS", indel = 2, expcost = 0.5),
                  2)

# Compare timing of statesgp$
comp_time <- round(seqCompare(seq53, seqdata2 = seq00, stat = "all", 
                              method = "CHI2", step = 101),
                   2)

# Compare duration in statesgp$
comp_dur <- round(seqCompare(seq53, seqdata2 = seq00, stat = "all", 
...
gp_mt53 <- round(seqmeant(seq53),2) #, serr = T for calculating Var, SD, SE
gp_mt00 <- round(seqmeant(seq00),2)

colnames(gp_mt53) <- "1953"
