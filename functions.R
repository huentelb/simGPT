...
write_socsim_rates_HFD <- function(Country) {
  ...
  ASFR <- 
    ...
    map_df(~ add_row(.x,
                     Year = unique(.x$Year), 
                     Age = 0, Age_up = 14,  Month = 0, ASFR_mo = 0.0, 
                     .before = 1)) %>% 
    group_by(Year) %>% 
    group_split() %>% 
    map_df(~ add_row(.x, 
  ...
  for(Year in years) {
    ...
    n <- which(Year == years)
    n_row <- (n-1)*39 + rows_ageF # Number of rows per Year after adding rows 0
    ...
    cat(c("** NB: The original HFD annual rates have been converted into monthly rates", "\n"), 
        file = outfilename)
    cat(c("** The open age interval (50+) is limited to one year [50-51)", "\n"), 
        file = outfilename)
    cat("\n", file = outfilename)
